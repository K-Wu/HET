from ...transforms.InterOpSSA import serializer
...
def remove_comment_and_whitespace(lines):
    lines_after_filter = []
    for line in lines:
        if line.find("//") != -1:
            line = line[: line.find("//")]
        line = line.strip()
        line = serializer.strip_white_spaces(line)
...
if __name__ == "__main__":
    for filename in filenames:
        with open(filename) as fd:
            instructions = serializer.loads(fd)
            actual_str = serializer.dumps(instructions)
        with open(filename) as fd:
