do_all_cases() {
      ...
      for d in ${Datasets[@]}; do
        for c_idx in ${!CompactFlag[@]}; do
          for mf_idx in ${!MulFlag[@]}; do
            ...
            echo "python -m python.$m.train -d $d --num_layers 1  --full_graph_training --num_classes $dim  --n_infeat $dim  --num_heads 1 $c $mf"
            eval $2 python -m python.$m.train -d $d --num_layers 1 --full_graph_training --num_classes $dim --n_infeat $dim --num_heads 1 $c $mf $3
          done
        done
      done
    done
  ...
  for d in ${Datasets[@]}; do
    for c_idx in ${!CompactFlag[@]}; do
      ...
      echo "python -m python.RGCN.RGCNSingleLayerSeparateCOO -d $d --num_layers 1  --full_graph_training --num_classes $dim --n_infeat $dim $c"
      eval $2 python -m python.RGCN.RGCNSingleLayerSeparateCOO -d $d --num_layers 1 --full_graph_training --num_classes $dim --n_infeat $dim $c $3
    done
  done
  done
}
