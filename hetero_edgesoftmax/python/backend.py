...
def rgcn_layer1_csr(graph, x, weight, norm):
    ...
    ret = th.zeros((row_ptr.numel()-1, weight.size(2)), dtype=weight.dtype, device=weight.device, requires_grad=True)
    return RgcnSecondLayerCSR.apply(row_ptr, col_idx, eids, reltypes, x, weight, norm, ret)