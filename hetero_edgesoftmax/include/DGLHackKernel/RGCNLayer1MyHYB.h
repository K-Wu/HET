...
// bgs:
template<typename Idx, typename DType, int ELL_logical_width, int ELL_physical_width>
__global__ void RgcnLayer1MyHYBKernelImpl(const Idx* ellcolidx_data, 
  const Idx* ellreltype_data, 
  const Idx* elleids_data,
  const Idx* ranges, 
  const Idx* src_ids, 
  const Idx* eids, 
  const Idx* types, 
...
template </*int XPU, */typename Idx, typename DType, int ELL_logical_width, int ELL_physical_width>
void RgcnLayer1MyHYBImpl(
    //GraphRef graph,
    //MyHeteroIntegratedCSR<int32_t, thrust::device_allocator<int32_t>> csr,
    MyHyb<int32_t, thrust::device_allocator<int32_t>, MyHeteroIntegratedCSR<int32_t, thrust::device_allocator<int32_t>>> hyb,
    ...
    //MySimpleNDArray<Idx, thrust::device_allocator<Idx>> eids
        ...
        auto ellcolidx_data = static_cast<Idx*>(thrust::raw_pointer_cast(hyb.ELLColIdx.data()));
        auto ellreltype_data = static_cast<Idx*>(thrust::raw_pointer_cast(hyb.ELLRelType.data()));
        auto elleids_data = static_cast<Idx*>(thrust::raw_pointer_cast(hyb.ELLEids.data()));

        auto range_data = static_cast<Idx*>(thrust::raw_pointer_cast(hyb.csr.row_ptr.data()));
        auto ids_data = static_cast<Idx*>(thrust::raw_pointer_cast(hyb.csr.col_idx.data()));
        //auto eids_data = eids.Ptr();
        auto eids_data = static_cast<Idx*>(thrust::raw_pointer_cast(hyb.csr.eids.data()));
        auto typeids_data = static_cast<Idx*>(thrust::raw_pointer_cast(hyb.csr.rel_type.data()));
        ...
        RgcnLayer1MyHYBKernelImpl<Idx, DType, ELL_logical_width, ELL_physical_width><<<nblks, nthrs/*, 0, thr_entry->stream*/>>>
            (ellcolidx_data, ellreltype_data, elleids_data, range_data, ids_data, eids_data, typeids_data, hidden_data, weight_data, norm_data, ret_data, num_nodes, feat_len_y, feat_len_x, ntypes);
        cuda_err_chk(cudaPeekAtLastError());
        cuda_err_chk(cudaDeviceSynchronize());
        std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
        std::cout << "MyHYB RGCN Layer 1 forward time: " << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count() << " ms"<<std::endl;
