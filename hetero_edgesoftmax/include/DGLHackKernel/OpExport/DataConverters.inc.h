...
std::vector<at::Tensor> convert_integrated_csr_to_separate_csr(
  ...
  auto csr_separate = ToSeparateCSR<int64_t>(csr);

  at::Tensor separate_relptr =
      at::empty({csr_separate.num_rels + 1}, csr_reltypes.options());
  ...
  separate_relptr.at(0) = 0;
  for (size_t rel_idx = 0; rel_idx < csr_separate.num_nnzs.size(); rel_idx++) {
    separate_relptr.at(rel_idx + 1) =
        separate_relptr.at(rel_idx) + csr_separate.num_nnzs[rel_idx];
  }

  thrust::copy(csr_separate.rowptr.begin(), csr_separate.rowptr.end(),
...
std::vector<at::Tensor> convert_integrated_csr_to_separate_coo(
  ...
  at::Tensor separate_relptr =
      at::empty({csr.num_rels + 1}, csr_reltypes.options());
