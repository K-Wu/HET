...
__device__ __forceinline__ static void func512_32_mysgemm_exec(int m, int n, int k, float4 *__restrict__ attention, float *__restrict__ Adata, float *__restrict__ Bdata, float *__restrict__ Cdata, int *__restrict__ node_indices, int *__restrict__ dense_edges_per_src_node, int *__restrict__ num_dense_edges_per_src_node, int *__restrict__ starting_pos_dense_edges_per_src_node, int *__restrict__ eids, int BcolBias)
{
    assert(OUT_DIM == 256);
    assert(NUM_HEADS == 4);
    ...
    __builtin_assume(k == 64);assert(k==64);
    __builtin_assume(m == OUT_DIM);assert(m==OUT_DIM);
    __builtin_assume(m == 256); assert(m==256);// TODO: make the routine more general
/********************************************************************
 *
 * Compute C = A x B
...
#define B(idx_head, row, col) Bdata[(idx_head * K) + (row) + (node_indices_shmem[col-BcolBias]) * OUT_DIM]
#define B_before_metadata_cache(idx_head, row, col) Bdata[(idx_head * K) + (row) + (node_indices[col]) * OUT_DIM]
#define C(idx_head, row, col) Cdata[(idx_head * K) + (row) + (col)*OUT_DIM]
    ...
    __shared__ float shmem_easy_output[TILE_SZ_B/*node idx*/][256/*element idx in 4 heads*/];
    ...
    if (threadIdx.x<TILE_SZ_B){
        if (threadIdx.x+BcolBias< n){
            ...
            num_dense_edges_per_src_node_reg_temp = num_dense_edges_per_src_node[curr_node_index];
            starting_pos_dense_edges_per_src_node_reg_temp = starting_pos_dense_edges_per_src_node[curr_node_index];
        }
    }
    ...
    // warp specialization scheme
    //if (threadIdx.x<512){
        shmem[0][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][(threadIdx.x % (TILE_SZ_A / TILE_NUM_HEAD)) / (8)][(threadIdx.x % (TILE_SZ_A / TILE_NUM_HEAD)) % (8)] = (shdmemLDBrowIdx < K && shdmemLDBcolIdx < n) ? B_before_metadata_cache(shdmemLDBheadIdx, shdmemLDBrowIdx, shdmemLDBcolIdx) : 0.0f;
        shmem[0][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][(threadIdx.x % (TILE_SZ_A / TILE_NUM_HEAD)) / (8) + 16][(threadIdx.x % (TILE_SZ_A / TILE_NUM_HEAD)) % (8)] = (shdmemLDBrowIdx < K && shdmemLDBcolIdx + 16 < n) ? B_before_metadata_cache(shdmemLDBheadIdx, shdmemLDBrowIdx, shdmemLDBcolIdx + 16) : 0.0f;
    //}
    ...
    #pragma unroll 2
                for (int shdmemColIdx = 0; shdmemColIdx < 16; shdmemColIdx++)
                {
                    ...
                    //TODO: optimize the shmem_output as no need to cater for C output coalescing
                    //TODO: improve second-step reuse by design the format to reuse global reads
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                     //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx] += reg0 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][0] /*shmem_val0[shdmemColIdx % 2]*/;
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                     //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx] += reg1 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][1] /*shmem_val1[shdmemColIdx % 2]*/;
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                     //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx]+= reg2 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][2] /*shmem_val2[shdmemColIdx % 2]*/;
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                     //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx]+= reg3 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][3] /*shmem_val3[shdmemColIdx % 2]*/;
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                     //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx]+= reg4 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][4] /*shmem_val4[shdmemColIdx % 2]*/;
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                     //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx]+= reg5 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][5] /*shmem_val5[shdmemColIdx % 2]*/;
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                     //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx]+= reg6 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][6] /*shmem_val6[shdmemColIdx % 2]*/;
                    /*C(ArowIdx / k, ArowIdx % k, CcolIdx)*/ /*shmem_output[shdmemColIdx][ArowIdx / 16][shdmemColIdxBias / 16][ArowIdx % 16]*/
                    //shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx] += reg7 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][7] /*shmem_val7[shdmemColIdx % 2]*/;

                    
                    float accum = reg0 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][0]+
                    reg1 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][1]+
                    reg2 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][2]+
                    reg3 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][3]+
                    reg4 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][4]+
                    reg5 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][5]+
                    reg6 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][6]+
                    reg7 * shmem[i % 2][threadIdx.x / (TILE_SZ_A / TILE_NUM_HEAD)][shdmemColIdx + shdmemColIdxBias][7];
                    // lane idx and 16+idx update the same location, therefore shuffle then merge write to the same location
                    accum += __shfl_down_sync(-1, accum, 16);
                    if (threadIdx.x%32<16){
                        shmem_easy_output[(shdmemColIdx*2)+shdmemColIdxBias / 16][ArowIdx]+= accum;
                    }
    ...
    // TODO: here warps write TILE_SZ_B-long metadata arrays cache to shared memory

    if (threadIdx.x<TILE_SZ_B){
        num_dense_edges_per_src_node_shmem[threadIdx.x] = num_dense_edges_per_src_node_reg_temp;
        starting_pos_dense_edges_per_src_node_shmem[threadIdx.x] = starting_pos_dense_edges_per_src_node_reg_temp;
    }
...
__global__ void HGTExperimentalEdgeAttentionFusedCOOKernel_512_32(int num_relations, float4 *__restrict__ attention, float *__restrict__ node_input_data, float *__restrict__ relation_attention_matrices,
                                                                  ...
                                                                  int *__restrict__ dense_edges_per_src_node, int *__restrict__ num_dense_edges_per_src_node, int *__restrict__ starting_pos_dense_edges_per_src_node, int *__restrict__ eids)
{
    ...
    // TODO: improve locality by each block working on multiple launches of func512_32_mysgemm_exec rather than one launch of func512_32_mysgemm_exec for each block
...
__global__ void HGTExperimentalEdgeAttentionResidueCSR(int num_relations, int num_rows, float4 *__restrict__ attention, float *__restrict__ node_input_data, float *__restrict__ weights, int *__restrict__ num_nnzs_per_relation, int *__restrict__ rel_ptr, int *__restrict__ row_ptr, int *__restrict__ col_idx, int *__restrict__ eids, int *__restrict__ exclusive_scan_numBlocks_per_relationship)
{
    ...
    for (int edgeIdx = blockIdx.x * NUM_EDGES_TO_PROCESS_PER_BLOCK - rel_ptr[relation_idx]; edgeIdx < my_min<>((int) (1+blockIdx.x) * NUM_EDGES_TO_PROCESS_PER_BLOCK - rel_ptr[relation_idx],num_nnzs_per_relation[relation_idx]); edgeIdx++)
    {
        ...
        if (threadIdx.x % 32 == 0)
        {
            atomicAdd(&(reinterpret_cast<float*>(attention)[curr_eid * 4 + idxHead]), att_val); // TODO: needs to clear to zero at the beginning
        }
