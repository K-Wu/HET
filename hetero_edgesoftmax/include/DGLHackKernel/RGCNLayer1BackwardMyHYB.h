...
__global__ void RgcnLayer1BackwardMyHYBKernelImpl(const Idx* ellcolidx_data, 
...
void RgcnLayer1BackwardMyHYBImpl(
    ...
    MyHyb<int32_t, thrust::device_allocator<int32_t>, MyHeteroIntegratedCSR<int32_t, thrust::device_allocator<int32_t>>> transposed_hyb,
    ...
    //MySimpleNDArray<Idx, thrust::device_allocator<Idx>> eids
        ...
        auto ellcolidx_data = static_cast<Idx*>(thrust::raw_pointer_cast(transposed_hyb.ELLColIdx.data()));
        auto ellreltype_data = static_cast<Idx*>(thrust::raw_pointer_cast(transposed_hyb.ELLRelType.data()));
        auto elleids_data = static_cast<Idx*>(thrust::raw_pointer_cast(transposed_hyb.ELLEids.data()));

        auto range_data = static_cast<Idx*>(thrust::raw_pointer_cast(transposed_hyb.csr.row_ptr.data()));
        auto ids_data = static_cast<Idx*>(thrust::raw_pointer_cast(transposed_hyb.csr.col_idx.data()));
        //auto eids_data = eids.Ptr();
        auto eids_data = static_cast<Idx*>(thrust::raw_pointer_cast(transposed_hyb.csr.eids.data()));
        auto typeids_data = static_cast<Idx*>(thrust::raw_pointer_cast(transposed_hyb.csr.rel_type.data()));
        ...
        RgcnLayer1BackwardMyHYBKernelImpl<Idx, DType, ELL_logical_width, ELL_physical_width>
            <<<nblks, nthrs/*, 0, thr_entry->stream*/>>>
            (ellcolidx_data, ellreltype_data, elleids_data, range_data, ids_data, eids_data, typeids_data,
        ...
        std::cout << "MyHYB RGCN Layer 1 backward time: " << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count() << " ms"<<std::endl;
