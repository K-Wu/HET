...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4FirstStageConcatenatedCSCKernel(int num_relations, int col_idx, float **__restrict__ outNodes_per_relation, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ relation,
...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4SecondStageConcatenatedCSCKernel(int col_idx, float *__restrict__ outEdge, float **__restrict__ outNodes_per_relation, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
...
template <int num_max_relations>
__device__ __forceinline__ void _EdgeSoftmax_4FirstStageConcatenatedCSCKernel(int num_relations, int beg_col_idx, int stride, float **__restrict__ outNodes_per_relation, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ matRelation,
                                                                            float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride)
    {
        _perRow_EdgeSoftmax_4FirstStageConcatenatedCSCKernel<num_max_relations>(num_relations, col_idx, outNodes_per_relation, num_cols, nnz, matCols, matRows, matRelation, edge_input_data, mus);
    }
}

__device__ __forceinline__ void _EdgeSoftmax_4SecondStageConcatenatedCSCKernel(int beg_col_idx, int stride, float *__restrict__ outEdge, float **__restrict__ outNodes_per_relation, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ matRelation,
                                                                             float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride)
    {
        _perRow_EdgeSoftmax_4SecondStageConcatenatedCSCKernel(col_idx, outEdge, outNodes_per_relation, num_cols, nnz, matCols, matRows, matRelation, edge_input_data, mus);
    }
}

template <int num_max_relations>
__global__ void EdgeSoftmax_4ConcatenatedCSCKernel(int num_relations, float *__restrict__ outEdges, float **__restrict__ outNodes_per_relation, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ matRelation,
                                                 float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    int beg_col_idx = blockIdx.x * blockDim.x + threadIdx.x;
    _EdgeSoftmax_4FirstStageConcatenatedCSCKernel<num_max_relations>(num_relations, beg_col_idx, blockDim.x * gridDim.x, outNodes_per_relation, num_cols, nnz, matCols, matRows, matRelation, edge_input_data, mus);
    _EdgeSoftmax_4SecondStageConcatenatedCSCKernel(beg_col_idx, blockDim.x * gridDim.x, outEdges, outNodes_per_relation, num_cols, nnz, matCols, matRows, matRelation, edge_input_data, mus);
}

std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4ConcatenatedCSCKernel(cusp::csr_matrix<int, int, cusp::device_memory> concatenated_csc_matrix, int num_relations, bool FlagInitWithRandomValue)
{
