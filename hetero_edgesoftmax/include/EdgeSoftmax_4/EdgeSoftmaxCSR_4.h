...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4FirstStageConcatenatedCSRKernel(int row_idx, float **__restrict__ outNodes_per_relation, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ relation,
...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4SecondStageConcatenatedCSRKernel(int row_idx, float *__restrict__ outEdge, float **__restrict__ outNodes_per_relation, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
...
__device__ __forceinline__ void _EdgeSoftmax_4FirstStageConcatenatedCSRKernel(int beg_row_idx, int stride, float **__restrict__ outNodes_per_relation, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ matRelation,
                                                                            float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    for (int row_idx = beg_row_idx; row_idx < num_rows; row_idx += stride)
    {
        _perRow_EdgeSoftmax_4FirstStageConcatenatedCSRKernel(row_idx, outNodes_per_relation, num_rows, nnz, matCols, matRows, matRelation, edge_input_data, mus);
    }
}

__device__ __forceinline__ void _EdgeSoftmax_4SecondStageConcatenatedCSRKernel(int beg_row_idx, int stride, float *__restrict__ outEdge, float **__restrict__ outNodes_per_relation, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ matRelation,
                                                                             float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    for (int row_idx = beg_row_idx; row_idx < num_rows; row_idx += stride)
    {
        _perRow_EdgeSoftmax_4SecondStageConcatenatedCSRKernel(row_idx, outEdge, outNodes_per_relation, num_rows, nnz, matCols, matRows, matRelation, edge_input_data, mus);
    }
}

__global__ void EdgeSoftmax_4ConcatenatedCSRKernel(float *__restrict__ outEdges, float **__restrict__ outNodes_per_relation, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows, int *__restrict__ matRelation,
                                                 float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    int beg_row_idx = blockIdx.x * blockDim.x + threadIdx.x;
    _EdgeSoftmax_4FirstStageConcatenatedCSRKernel(beg_row_idx, blockDim.x * gridDim.x, outNodes_per_relation, num_rows, nnz, matCols, matRows, matRelation, edge_input_data, mus);
    _EdgeSoftmax_4SecondStageConcatenatedCSRKernel(beg_row_idx, blockDim.x * gridDim.x, outEdges, outNodes_per_relation, num_rows, nnz, matCols, matRows, matRelation, edge_input_data, mus);
}

std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4ConcatenatedCSRKernel(cusp::csr_matrix<int, int, cusp::device_memory> concatenated_csr_matrix, int num_relations, bool FlagInitWithRandomValue)
{
