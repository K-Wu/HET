...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4FirstStageCSCKernel(int num_relations, int col_idx, float *__restrict__ outNode, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4SecondStageCSCKernel(int col_idx, float *__restrict__ outEdge, float *__restrict__ outNode, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
                                                                        float *__restrict__ edge_input_data, float mu)
{
    //@@ insert spmv kernel for csr format
    if (col_idx >= num_cols)
...
template <int num_max_relations>
__device__ __forceinline__ void _EdgeSoftmax_4FirstStageCSCKernel(int num_relations, int beg_col_idx, int stride, float *__restrict__ outNode, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
                                                                float *__restrict__ edge_input_data, float mu)
{
    for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride)
    {
        _perRow_EdgeSoftmax_4FirstStageCSCKernel<num_max_relations>(num_relations, col_idx, outNode, num_cols, nnz, matCols, matRows, edge_input_data, mu);
    }
}

__device__ __forceinline__ void _EdgeSoftmax_4SecondStageCSCKernel(int beg_col_idx, int stride, float *__restrict__ outEdge, float *__restrict__ outNode, int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
                                                                 float *__restrict__ edge_input_data, float mu)
{
    for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride)
    {
        _perRow_EdgeSoftmax_4SecondStageCSCKernel(col_idx, outEdge, outNode, num_cols, nnz, matCols, matRows, edge_input_data, mu);
    }
}

//template <int BLOCK_PER_SM>
template <int num_max_relations>
__global__ void EdgeSoftmax_4MultiCSCsKernel(int *__restrict__ blockid_relation_id, int *__restrict__ beg_col_idxes, int *__restrict__ num_block_for_same_relation, float **__restrict__ outNode_per_relation, float *__restrict__ outEdge, int num_relations, int num_cols, int *__restrict__ nnzs, int **__restrict__ matCols, int **__restrict__ matRows,
                                           float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    ...
    _EdgeSoftmax_4FirstStageCSCKernel<num_max_relations>(num_relations, beg_col_idxes[blockIdx.x] + threadIdx.x, num_block_for_same_relation[blockIdx.x] * blockDim.x, outNode_per_relation[relation_id], num_cols, nnzs[relation_id], matCols[relation_id], matRows[relation_id], &edge_input_data[offset], mus[relation_id]);
    _EdgeSoftmax_4SecondStageCSCKernel(beg_col_idxes[blockIdx.x] + threadIdx.x, num_block_for_same_relation[blockIdx.x] * blockDim.x, &outEdge[offset], outNode_per_relation[relation_id], num_cols, nnzs[relation_id], matCols[relation_id], matRows[relation_id], &edge_input_data[offset], mus[relation_id]);
}

std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4MultiCSCsKernel(std::vector<cusp::csr_matrix<int, int, cusp::device_memory>> csc_matrices, bool FlagInitWithRandomValue)
{
