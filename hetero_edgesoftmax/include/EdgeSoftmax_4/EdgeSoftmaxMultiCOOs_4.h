...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4FirstStageCOOKernel(int edge_idx, float *__restrict__ outNode, int nnz, int *__restrict__ matCols,
...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4SecondStageCOOKernel(int edge_idx, float *__restrict__ outEdge, float *__restrict__ outNode, int nnz, int *__restrict__ matCols,
                                                                        float *__restrict__ edge_input_data, float mu)
{
    //@@ insert spmv kernel for coo format
...
__device__ __forceinline__ void _EdgeSoftmax_4FirstStageCOOKernel(int beg_edge_idx, int stride, float *__restrict__ outNode, int nnz, int *__restrict__ matCols,
                                                                float *__restrict__ edge_input_data, float mu)
{
    for (int edge_idx = beg_edge_idx; edge_idx < nnz; edge_idx += stride)
    {
        _perRow_EdgeSoftmax_4FirstStageCOOKernel(edge_idx, outNode, nnz, matCols, edge_input_data, mu);
    }
}

__device__ __forceinline__ void _EdgeSoftmax_4SecondStageCOOKernel(int beg_edge_idx, int stride, float *__restrict__ outEdge, float *__restrict__ outNode, int nnz, int *__restrict__ matCols,
                                                                 float *__restrict__ edge_input_data, float mu)
{
    for (int edge_idx = beg_edge_idx; edge_idx < nnz; edge_idx += stride)
    {
        _perRow_EdgeSoftmax_4SecondStageCOOKernel(edge_idx, outEdge, outNode, nnz, matCols, edge_input_data, mu);
    }
}

//template <int BLOCK_PER_SM>
__global__ void EdgeSoftmax_4MultiCOOsKernel(int *__restrict__ blockid_relation_id, int *__restrict__ beg_edge_idxes, int *__restrict__ num_block_for_same_relation, float **__restrict__ outNode_per_relation, float *__restrict__ outEdge, int num_relations, int *__restrict__ nnzs, int **__restrict__ matCols,
                                           float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    ...
    _EdgeSoftmax_4FirstStageCOOKernel(beg_edge_idxes[blockIdx.x] + threadIdx.x, num_block_for_same_relation[blockIdx.x] * blockDim.x, outNode_per_relation[relation_id], nnzs[relation_id], matCols[relation_id], &edge_input_data[offset], mus[relation_id]);
    _EdgeSoftmax_4SecondStageCOOKernel(beg_edge_idxes[blockIdx.x] + threadIdx.x, num_block_for_same_relation[blockIdx.x] * blockDim.x, &outEdge[offset], outNode_per_relation[relation_id], nnzs[relation_id], matCols[relation_id], &edge_input_data[offset], mus[relation_id]);
}

std::vector<thrust::device_vector<float>> _doGPUEdgeSoftmax_4MultiCOOsKernel(int num_nodes, int num_relations, std::vector<cusp::coo_matrix<int, int, cusp::device_memory>::column_indices_array_type> coo_matrices_column_indices, bool FlagInitWithRandomValue)
{
...
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4MultiCOOsKernel(std::vector<matrix_type> coo_matrices, bool FlagInitWithRandomValue)
{
    assert(0);
}

template <>
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4MultiCOOsKernel<cusp::coo_matrix<int, int, cusp::device_memory>>(std::vector<cusp::coo_matrix<int, int, cusp::device_memory>> coo_matrices, bool FlagInitWithRandomValue)
{
    ...
    return _doGPUEdgeSoftmax_4MultiCOOsKernel(coo_matrices[0].num_rows, coo_matrices.size(), coo_matrices_column_indices, FlagInitWithRandomValue);
}

template <>
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4MultiCOOsKernel<cusp::csr_matrix<int, int, cusp::device_memory>>(std::vector<cusp::csr_matrix<int, int, cusp::device_memory>> csr_matrices, bool FlagInitWithRandomValue)
{
    ...
    return _doGPUEdgeSoftmax_4MultiCOOsKernel(csr_matrices[0].num_rows, csr_matrices.size(), csr_matrices_column_indices, FlagInitWithRandomValue);
}