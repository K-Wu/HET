...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4FirstStageCSRKernel(int row_idx, float *__restrict__ outNode, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4SecondStageCSRKernel(int row_idx, float *__restrict__ outEdge, float *__restrict__ outNode, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
                                                                        float *__restrict__ edge_input_data, float mu)
{
    //@@ insert spmv kernel for csr format
    if (row_idx >= num_rows)
...
__device__ __forceinline__ void _EdgeSoftmax_4FirstStageCSRKernel(int beg_row_idx, int stride, float *__restrict__ outNode, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
                                                                float *__restrict__ edge_input_data, float mu)
{
    for (int row_idx = beg_row_idx; row_idx < num_rows; row_idx += stride)
    {
        _perRow_EdgeSoftmax_4FirstStageCSRKernel(row_idx, outNode, num_rows, nnz, matCols, matRows, edge_input_data, mu);
    }
}

__device__ __forceinline__ void _EdgeSoftmax_4SecondStageCSRKernel(int beg_row_idx, int stride, float *__restrict__ outEdge, float *__restrict__ outNode, int num_rows, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
                                                                 float *__restrict__ edge_input_data, float mu)
{
    for (int row_idx = beg_row_idx; row_idx < num_rows; row_idx += stride)
    {
        _perRow_EdgeSoftmax_4SecondStageCSRKernel(row_idx, outEdge, outNode, num_rows, nnz, matCols, matRows, edge_input_data, mu);
    }
}

//template <int BLOCK_PER_SM>
__global__ void EdgeSoftmax_4MultiCSRsKernel(int *__restrict__ blockid_relation_id, int *__restrict__ beg_row_idxes, int *__restrict__ num_block_for_same_relation, float **__restrict__ outNode_per_relation, float *__restrict__ outEdge, int num_relations, int num_rows, int *__restrict__ nnzs, int **__restrict__ matCols, int **__restrict__ matRows,
                                           float *__restrict__ edge_input_data, float *__restrict__ mus)
{
    ...
    _EdgeSoftmax_4FirstStageCSRKernel(beg_row_idxes[blockIdx.x] + threadIdx.x, num_block_for_same_relation[blockIdx.x] * blockDim.x, outNode_per_relation[relation_id], num_rows, nnzs[relation_id], matCols[relation_id], matRows[relation_id], &edge_input_data[offset], mus[relation_id]);
    _EdgeSoftmax_4SecondStageCSRKernel(beg_row_idxes[blockIdx.x] + threadIdx.x, num_block_for_same_relation[blockIdx.x] * blockDim.x, &outEdge[offset], outNode_per_relation[relation_id], num_rows, nnzs[relation_id], matCols[relation_id], matRows[relation_id], &edge_input_data[offset], mus[relation_id]);
}

std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4MultiCSRsKernel(std::vector<cusp::csr_matrix<int, int, cusp::device_memory>> csr_matrices, bool FlagInitWithRandomValue)
{
    ...
    std::vector<thrust::device_vector<float>> outNodes_per_relation_vect_vect(csr_matrices.size(), thrust::device_vector<float>(csr_matrices[0].num_rows, 0));
    thrust::device_vector<float *> outNodes_per_relation_vect;
    size_t total_nnzs = 0;
    for (int idx_relation = 0; idx_relation < csr_matrices.size(); idx_relation++)
    {

        matCols_vect[idx_relation] = thrust::raw_pointer_cast(csr_matrices[idx_relation].column_indices.data());
        matRows_vect[idx_relation] = thrust::raw_pointer_cast(csr_matrices[idx_relation].row_offsets.data());
    ...
    {
        int num_blocks_for_this_and_prev_relation = (idx_relationship + 1 + 0.0) / (csr_matrices.size() + 0.0) * RTX_3090_GRIDSIZE;
    ...
    for (int idx_block = 0; idx_block < RTX_3090_GRIDSIZE; idx_block++)
    {
        ...
        {
            assert(curr_beg_row_idx / RTX_3090_BLOCKSIZE == num_blocks_for_same_relation_vect[idx_curr_relation]);
            idx_curr_relation++;
            curr_beg_row_idx = 0;
        }
