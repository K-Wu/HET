...
__device__ __forceinline__ void _perRow_EdgeSoftmaxFirstStageCSRKernel(
...
__device__ __forceinline__ void _perRow_EdgeSoftmaxSecondStageCSRKernel(
...
__device__ __forceinline__ void _EdgeSoftmaxFirstStageCSRKernel(
  ...
  for (int row_idx = beg_row_idx; row_idx < num_rows; row_idx += stride) {
    _perRow_EdgeSoftmaxFirstStageCSRKernel(
        row_idx, outNode, num_rows, nnz, matCols, matRows, edge_input_data, mu);
  }
}

__device__ __forceinline__ void _EdgeSoftmaxSecondStageCSRKernel(
    int beg_row_idx, int stride, float *__restrict__ outEdge,
    float *__restrict__ outNode, int num_rows, int nnz,
    int *__restrict__ matCols, int *__restrict__ matRows,
    float *__restrict__ edge_input_data, float mu) {
  for (int row_idx = beg_row_idx; row_idx < num_rows; row_idx += stride) {
    _perRow_EdgeSoftmaxSecondStageCSRKernel(row_idx, outEdge, outNode, num_rows,
                                            nnz, matCols, matRows,
                                            edge_input_data, mu);
  }
}
...
__global__ void HET_EdgeSoftmaxMultiCSRsKernel(
  ...
  _EdgeSoftmaxFirstStageCSRKernel(
      ...
      mus[relation_id]);
  _EdgeSoftmaxSecondStageCSRKernel(
      beg_row_idxes[blockIdx.x] + threadIdx.x,
...
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmaxMultiCSRsKernel(
  ...
  HET_EdgeSoftmaxMultiCSRsKernel<<<grid, block>>>(
  ...
  std::chrono::high_resolution_clock::time_point t2 =
      std::chrono::high_resolution_clock::now();
  std::cout
      << "GPU HET_EdgeSoftmaxMultiCSRsKernel time: "
