...
_perRow_EdgeSoftmaxFirstStageConcatenatedCOOKernel(
    int edge_idx, float **__restrict__ outNodes_per_relation, int nnz,
...
__device__ __forceinline__ void
_perRow_EdgeSoftmaxSecondStageConcatenatedCOOKernel(
    int edge_idx, float *__restrict__ outEdge,
    float **__restrict__ outNodes_per_relation, int nnz,
    int *__restrict__ matCols, int *__restrict__ relation,
    float *__restrict__ edge_input_data, float *__restrict__ mus) {
  //@@ insert spmv kernel for coo format
...
__device__ __forceinline__ void _EdgeSoftmaxFirstStageConcatenatedCOOKernel(
    int beg_edge_idx, int stride, float **__restrict__ outNodes_per_relation,
...
__device__ __forceinline__ void _EdgeSoftmaxSecondStageConcatenatedCOOKernel(
...
_doGPUEdgeSoftmaxConcatenatedCOOKernel(
    int num_nodes,
  ...
  cudaMalloc((void **)&edge_input_data,
             sizeof(float) * concatenated_coo_matrix_column_indices.size());
  ...
  HET_EdgeSoftmaxConcatenatedCOOKernel<<<grid, block>>>(
  ...
  std::chrono::high_resolution_clock::time_point t2 =
      std::chrono::high_resolution_clock::now();
  std::cout
      << "GPU doGPUEdgeSoftmaxConcatenatedCOOKernel time: "
...
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmaxConcatenatedCOOKernel(
    cusp::coo_matrix<int, int, cusp::device_memory> concatenated_coo_matrix,
    int num_relations, bool FlagInitWithRandomValue) {
  return _doGPUEdgeSoftmaxConcatenatedCOOKernel(
