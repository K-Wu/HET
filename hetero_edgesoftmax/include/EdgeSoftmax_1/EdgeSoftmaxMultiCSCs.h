...
__device__ __forceinline__ void _perRow_EdgeSoftmaxFirstStageCSCKernel(
...
__device__ __forceinline__ void _perRow_EdgeSoftmaxSecondStageCSCKernel(
...
template <int num_max_relations>
__device__ __forceinline__ void _EdgeSoftmaxFirstStageCSCKernel(
  ...
  for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride) {
    _perRow_EdgeSoftmaxFirstStageCSCKernel<num_max_relations>(
        num_relations, col_idx, outNode, num_cols, nnz, matCols, matRows,
        edge_input_data, mu);
  }
}

__device__ __forceinline__ void _EdgeSoftmaxSecondStageCSCKernel(
    ...
    float *__restrict__ edge_input_data, float mu) {
  for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride) {
    _perRow_EdgeSoftmaxSecondStageCSCKernel(col_idx, outEdge, outNode, num_cols,
                                            nnz, matCols, matRows,
                                            edge_input_data, mu);
  }
}
...
template <int num_max_relations>
__global__ void HET_EdgeSoftmaxMultiCSCsKernel(
  ...
  _EdgeSoftmaxFirstStageCSCKernel<num_max_relations>(
      ...
      mus[relation_id]);
  _EdgeSoftmaxSecondStageCSCKernel(
...
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmaxMultiCSCsKernel(
  ...
  HET_EdgeSoftmaxMultiCSCsKernel<NUM_MAX_RELATIONS><<<grid, block>>>(
  ...
  std::chrono::high_resolution_clock::time_point t2 =
      std::chrono::high_resolution_clock::now();
  std::cout
      << "GPU HET_EdgeSoftmaxMultiCSCsKernel time: "
