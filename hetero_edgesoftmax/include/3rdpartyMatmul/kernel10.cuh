#pragma once
// from
// https://github.com/yzhaiustc/Optimizing-SGEMM-on-NVIDIA-Turing-GPUs/blob/main/include/kernel10.cuh
// A, B, and C are all column-major.
// WARNING: this kernel does not take into consideration cases where size of
// matrix is not a multiple of the tile size
// TODO: transposition needed but for performance test, we may ignore
// transposition and use the original raw data region anyway
// TODO: padding needed as vectorized load/store does not consider cases if the
// matrix #rows/cols is not multiple of vector length
...
__global__ __launch_bounds__(256) void mysgemm_v10(int M, int N, int K,
  ...
  vload(pref_Av, &A(row_a, col_a))
      vload(pref_Bv, &B(row_b, col_b))((float4*)sa10)[tx] = pref_Av;
  sb10(col_b, row_b) = pref_Bv.x;
  sb10(col_b, row_b + 1) = pref_Bv.y;
  sb10(col_b, row_b + 2) = pref_Bv.z;
  sb10(col_b, row_b + 3) = pref_Bv.w;
  __syncthreads();
  vload(Av1[0], &sa10(row_c, 0)) vload(Av2[0], &sa10(row_c + 4, 0))
      vload(Bv1[0], &sb10(col_c, 0)) vload(
          Bv2[0], &sb10(col_c + 4, 0)) for (int k_count = 0; k_count < K_upper;
                                            k_count++) {
...
#pragma unroll
      ...
      vload(
          Av1[(inner_k_count + 1) & 1],
          &sa10(row_c, next_inner_k_count)) vload(Av2[(inner_k_count + 1) & 1],
                                                  &sa10(row_c + 4,
                                                        next_inner_k_count))
    ...
    __syncthreads();
    vload(Av1[0], &sa10(row_c, 0)) vload(Av2[0], &sa10(row_c + 4, 0))
        vload(Bv1[0], &sb10(col_c, 0)) vload(Bv2[0], &sb10(col_c + 4, 0))
  }
