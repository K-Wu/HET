...
#define C(idx_head, row, col) C[(idx_head * k) + (row) + (col)*m]
    ...
    for (int i = 0; i < (k + 8 - 1) / (8); i++)
    {
        ...
        reg0 = (ArowIdx < m && k > i * 8) ? A(ArowIdx / k, ArowIdx % k, i * 8) : 0.0f;
        reg1 = (ArowIdx < m && k > i * 8 + 1) ? A(ArowIdx / k, ArowIdx % k, i * 8 + 1) : 0.0f;
        reg2 = (ArowIdx < m && k > i * 8 + 2) ? A(ArowIdx / k, ArowIdx % k, i * 8 + 2) : 0.0f;
        reg3 = (ArowIdx < m && k > i * 8 + 3) ? A(ArowIdx / k, ArowIdx % k, i * 8 + 3) : 0.0f;
        reg4 = (ArowIdx < m && k > i * 8 + 4) ? A(ArowIdx / k, ArowIdx % k, i * 8 + 4) : 0.0f;
        reg5 = (ArowIdx < m && k > i * 8 + 5) ? A(ArowIdx / k, ArowIdx % k, i * 8 + 5) : 0.0f;
        reg6 = (ArowIdx < m && k > i * 8 + 6) ? A(ArowIdx / k, ArowIdx % k, i * 8 + 6) : 0.0f;
        reg7 = (ArowIdx < m && k > i * 8 + 7) ? A(ArowIdx / k, ArowIdx % k, i * 8 + 7) : 0.0f;
        // }
        ...
        if (ArowIdx < m)
        {
            // TODO: software pipelining to load data from shared memory for the next iteration
