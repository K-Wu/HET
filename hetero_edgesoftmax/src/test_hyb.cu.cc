...
int main() {
  ...
  npy::LoadArrayFromNumpy("data/ogbn_mag_0.1/writing_coo_2.npy", writing_shape,
                          fortran_order, writing_data);

  std::vector<int> max_idxes;
  max_idxes.push_back(
      *std::max_element(written_by_data.begin(), written_by_data.end()));
  ...
  cusp::coo_matrix<int, int, cusp::host_memory> written_by_coo_h(
      max_idx + 1, max_idx + 1, written_by_data.size() / 2);
  for (int idx = 0; idx < written_by_data.size() / 2; idx++) {
    written_by_coo_h.row_indices[idx] = written_by_data[idx];
    written_by_coo_h.column_indices[idx] =
        written_by_data[idx + written_by_data.size() / 2];
  }

  cusp::coo_matrix<int, int, cusp::host_memory> has_coo_h(
      max_idx + 1, max_idx + 1, has_data.size() / 2);
  ...
  cusp::coo_matrix<int, int, cusp::host_memory> cited_coo_h(
      max_idx + 1, max_idx + 1, cited_data.size() / 2);
  ...
  cusp::csr_matrix<int, int, cusp::host_memory> written_by_csr_h(
      written_by_coo_h);
  ...
  std::vector<cusp::csr_matrix<int, int, cusp::host_memory>> csr_matrices = {
      written_by_csr_h, has_csr_h,    is_about_csr_h,
  ...
  MyHeteroSeparateCSR<int, std::allocator<int>> myHeteroSeparateCSR(
      csr_matrices, eids);
  ...
  MyHeteroIntegratedCSR<int, std::allocator<int>> myHeteroIntegratedCSR =
      ToIntegratedCSR<std::allocator<int>, std::allocator<int>, int>(
          myHeteroSeparateCSR);
  MyHeteroIntegratedCSR<int, std::allocator<int>>
      myHeteroIntegratedCSR_transposed(myHeteroIntegratedCSR);
  myHeteroIntegratedCSR_transposed.Transpose();
  MyHeteroIntegratedCSR<int, std::allocator<int>> myHeteroIntegratedCSR2(
      myHeteroIntegratedCSR_transposed);
  myHeteroIntegratedCSR2.Transpose();
  assert(IsEqual<>(myHeteroIntegratedCSR, myHeteroIntegratedCSR2));

  MyHeteroSeparateCSR<int, std::allocator<int>> myHeteroSeparateCSR2 =
      ToSeparateCSR<std::allocator<int>, std::allocator<int>, int>(
          myHeteroIntegratedCSR);
  assert(IsEqual<>(myHeteroSeparateCSR, myHeteroSeparateCSR2));
  MyHyb<int, std::allocator<int>, MyHeteroSeparateCSR<int, std::allocator<int>>>
      myhyb = IntegratedCSRToHyb_CPU<int>(myHeteroIntegratedCSR, 5, 5, 10000);
  MyHyb<int, std::allocator<int>, MyHeteroSeparateCSR<int, std::allocator<int>>>
      myhyb2 = IntegratedCSRToHyb_CPU<int>(myHeteroIntegratedCSR, 5, 7, 10000);
