...
int main(int argc, char *argv[]) {
  ...
  cuda_err_chk(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking));
  CUBLAS_CHECK(cublasSetStream(cublasH, stream));

  /* step 2: copy data to device */
  cuda_err_chk(cudaMalloc(reinterpret_cast<void **>(&d_A),
                          sizeof(data_type) * A.size()));
  cuda_err_chk(cudaMalloc(reinterpret_cast<void **>(&d_B),
                          sizeof(data_type) * B.size()));
  cuda_err_chk(cudaMalloc(reinterpret_cast<void **>(&d_C),
                          sizeof(data_type) * C.size()));

  cuda_err_chk(cudaMemcpyAsync(d_A, A.data(), sizeof(data_type) * A.size(),
                               cudaMemcpyHostToDevice, stream));
  cuda_err_chk(cudaMemcpyAsync(d_B, B.data(), sizeof(data_type) * B.size(),
                               cudaMemcpyHostToDevice, stream));
  ...
  cuda_err_chk(cudaMemcpyAsync(C.data(), d_C, sizeof(data_type) * C.size(),
  ...
  /* free resources */
  cuda_err_chk(cudaFree(d_A));
  cuda_err_chk(cudaFree(d_B));
