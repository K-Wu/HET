...
class RgnnInnerProductNodeCompactAndNodeWithDirectIndexing(
...
def rgnn_inner_product_right_node(
    ...
    if compact_as_of_node_kind == 0:  # CompactAsOfNodeKind::Disabled
        ...
        ret = th.zeros(
            [
                separate_coo_original_dict["eids"].numel(),
        ...
        ret = th.zeros(
            (
                separate_coo_original_dict["rel_ptrs"][-1],
                right_node_vectors.size(1),
            ),
            dtype=right_node_vectors.dtype,
            device=right_node_vectors.device,
