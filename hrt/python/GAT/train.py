...
def GAT_train(args, single_layer_flag: bool):
    ...
    print(
        "WARNING: the original seastar loading dataset features is replaced"
        " with the seastar randomized features and labels instead."
    )
    print(
        "WARNING the original seastar train_mask utility is replaced with the"
        " seastar randomized train_mask instead."
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        # log for each step
        print(
            "Epoch {:05d} | Time(s) {:.4f} | train_acc {:.6f} | Used_Memory"
            " {:.6f} mb".format(
                epoch,
                run_time_this_epoch,
                train_acc,
...
def GAT_get_parser(single_layer_flag: bool):
    ...
    parser.add_argument(
        "--num_feats", type=int, default=128, help="input feature dim"
    )
