...
class HET_HGTLayerHetero(nn.Module):
    ...
    def __init__(
        ...
        self.fused_message_mean_aggregation_flag = (
        ...
        if self.multiply_among_weights_first_flag:
            assert num_heads == 1, (
                "unsupported num_heads>1 when"
                " multiply_among_weights_first_flag is True"
            )
    ...
    def forward(self, G, h):
        ...
        if self.multiply_among_weights_first_flag:
            ...
            k_relation_att_q_product = torch.bmm(
                ...
                q_per_canonical_etype.view(
                    -1, self.in_dim, self.d_k
                ).transpose(1, 2),
        ...
        if self.hgt_fused_attn_score_flag:
            ...
            if self.compact_as_of_node_flag:
                compact_as_of_node_kind = (
                    2 if self.compact_direct_indexing_flag else 1
                )
        ...
        if self.fused_message_mean_aggregation_flag:
            ...
            new_h = (
                B.hgt_full_graph_edge_softmax_and_message_mean_aggregation_csr(
                    G,
                    message_per_edge,
                    attn_score,
                    (self.relation_pri / self.sqrt_dk),
