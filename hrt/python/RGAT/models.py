...
class HET_RGATLayer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        assert num_rels > 1, (
            "dummy proof assertion num_rels should be larger than 1 normally"
            " when calling RGAT_HET"
        )
        # NB: RGAT model definition
        assert (
            out_feat % num_heads == 0
        ), "out_feat must be a multiple of num_heads"
    ...
    def forward(self, g, inputs: th.Tensor):
        ...
        if self.compact_as_of_node_flag:
            ...
            if self.compact_direct_indexing_flag:
                print(
                    "Warning: matmul currently ignore compact direct indexing"
                    " flag as it is unclear how to benefit from it"
                )
            ...
            feat_src_per_edge = B.rgnn_relational_matmul(
                {
                    "separate_coo_rel_ptrs": separate_coo_original_dict[
                        "rel_ptrs"
                    ],
                ...
                er = B.rgnn_relational_matmul(
                    {
                        ...
                        "separate_coo_eids": separate_coo_original_dict[
                            "eids"
                        ],
                    },
                    product_of_conv_weights_attn_r,
...
class HET_RGATModel(nn.Module):
    ...
    def init_encoder(self):
        ...
        final_layer_num_heads = (
            self.num_heads if self.num_hidden_layers == 0 else 1
        )
        # h2o
        self.layers.append(
            HET_RGATLayer(
