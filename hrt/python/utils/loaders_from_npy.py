...
def generic_load_data(dataset_path_and_name_prefix):
    ...
    edge_srcs = np.load(
        dataset_path_and_name_prefix + ".srcs.npy", allow_pickle=True
    )
    edge_dsts = np.load(
        dataset_path_and_name_prefix + ".dsts.npy", allow_pickle=True
    )
    edge_etypes = np.load(
        dataset_path_and_name_prefix + ".etypes.npy", allow_pickle=True
    )
    edge_referential_eids = np.load(
        dataset_path_and_name_prefix + ".referential_eids.npy",
        allow_pickle=True,
    )
...
def fetch_ogbnmag_raw_data():
    ...
    edge_srcs4 = (
        graph[0]["edge_index_dict"][("paper", "has_topic", "field_of_study")][
            0
        ]
        + graph[0]["num_nodes_dict"]["author"]
    )
...
def load_fb15k237(
    ...
    if infidel_sort_flag:
        print(
            "Warning: you are loading infidel sort data, see readme.md for"
            " details"
        )
