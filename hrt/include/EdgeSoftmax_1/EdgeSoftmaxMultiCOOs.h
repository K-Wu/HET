...
//     // map block_index from [0, block_num) to [0, block_num) but aim to keep
//     the same relation on the same streaming mulitiprocessor int block_index =
//     blockIdx.x; int sm_id = getsmid(); int sm_num = getnsmid(); assert(sm_num
//     == RTX_3090_SM_NUM || sm_num == RTX_2070MQ_SM_NUM);
//     assert(gridDim.x%sm_num == 0);
//     assert(block_index % sm_num == sm_id);
//     return sm_id*(gridDim.x/sm_num) + (block_index/sm_num);
// }

__device__ __forceinline__ void _perRow_EdgeSoftmaxFirstStageCOOKernel(
...
__device__ __forceinline__ void _perRow_EdgeSoftmaxSecondStageCOOKernel(
...
__device__ __forceinline__ void _EdgeSoftmaxFirstStageCOOKernel(
    int beg_edge_idx, int stride, float *__restrict__ outNode, int nnz,
    int *__restrict__ matCols, float *__restrict__ edge_input_data, float mu) {
  for (int edge_idx = beg_edge_idx; edge_idx < nnz; edge_idx += stride) {
    _perRow_EdgeSoftmaxFirstStageCOOKernel(edge_idx, outNode, nnz, matCols,
                                           edge_input_data, mu);
  }
}

__device__ __forceinline__ void _EdgeSoftmaxSecondStageCOOKernel(
  ...
  for (int edge_idx = beg_edge_idx; edge_idx < nnz; edge_idx += stride) {
    _perRow_EdgeSoftmaxSecondStageCOOKernel(edge_idx, outEdge, outNode, nnz,
                                            matCols, edge_input_data, mu);
  }
}

// template <int BLOCK_PER_SM>
__global__ void HET_EdgeSoftmaxMultiCOOsKernel(
  ...
  _EdgeSoftmaxFirstStageCOOKernel(
  ...
  _EdgeSoftmaxSecondStageCOOKernel(
      beg_edge_idxes[blockIdx.x] + threadIdx.x,
...
std::vector<thrust::device_vector<float>> _doGPUEdgeSoftmaxMultiCOOsKernel(
  ...
  HET_EdgeSoftmaxMultiCOOsKernel<<<grid, block>>>(
  ...
  std::chrono::high_resolution_clock::time_point t2 =
      std::chrono::high_resolution_clock::now();
  std::cout
      << "GPU HET_EdgeSoftmaxMultiCOOsKernel time: "
...
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmaxMultiCOOsKernel<
  ...
  return _doGPUEdgeSoftmaxMultiCOOsKernel(
