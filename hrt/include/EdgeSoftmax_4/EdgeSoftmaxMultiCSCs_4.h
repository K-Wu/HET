...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4FirstStageCSCKernel(
    int num_relations, int col_idx, float *__restrict__ outNode, int num_cols,
...
__device__ __forceinline__ void _perRow_EdgeSoftmax_4SecondStageCSCKernel(
    int col_idx, float *__restrict__ outEdge, float *__restrict__ outNode,
    int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
    float *__restrict__ edge_input_data, float mu) {
  //@@ insert spmv kernel for csr format
  if (col_idx >= num_cols) return;
  int col_start = matCols[col_idx];
...
template <int num_max_relations>
__device__ __forceinline__ void _EdgeSoftmax_4FirstStageCSCKernel(
    int num_relations, int beg_col_idx, int stride, float *__restrict__ outNode,
    int num_cols, int nnz, int *__restrict__ matCols, int *__restrict__ matRows,
    float *__restrict__ edge_input_data, float mu) {
  for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride) {
    _perRow_EdgeSoftmax_4FirstStageCSCKernel<num_max_relations>(
        num_relations, col_idx, outNode, num_cols, nnz, matCols, matRows,
        edge_input_data, mu);
  }
}

__device__ __forceinline__ void _EdgeSoftmax_4SecondStageCSCKernel(
    int beg_col_idx, int stride, float *__restrict__ outEdge,
    float *__restrict__ outNode, int num_cols, int nnz,
    int *__restrict__ matCols, int *__restrict__ matRows,
    float *__restrict__ edge_input_data, float mu) {
  for (int col_idx = beg_col_idx; col_idx < num_cols; col_idx += stride) {
    _perRow_EdgeSoftmax_4SecondStageCSCKernel(col_idx, outEdge, outNode,
                                              num_cols, nnz, matCols, matRows,
                                              edge_input_data, mu);
  }
}
...
template <int num_max_relations>
__global__ void HET_EdgeSoftmax_4MultiCSCsKernel(
    int *__restrict__ blockid_relation_id, int *__restrict__ beg_col_idxes,
    int *__restrict__ num_block_for_same_relation,
    float **__restrict__ outNode_per_relation, float *__restrict__ outEdge,
    int num_relations, int num_cols, int *__restrict__ nnzs,
    int **__restrict__ matCols, int **__restrict__ matRows,
  ...
  _EdgeSoftmax_4FirstStageCSCKernel<num_max_relations>(
      num_relations, beg_col_idxes[blockIdx.x] + threadIdx.x,
      num_block_for_same_relation[blockIdx.x] * blockDim.x,
      outNode_per_relation[relation_id], num_cols, nnzs[relation_id],
      matCols[relation_id], matRows[relation_id], &edge_input_data[offset],
      mus[relation_id]);
  _EdgeSoftmax_4SecondStageCSCKernel(
      beg_col_idxes[blockIdx.x] + threadIdx.x,
...
std::vector<thrust::device_vector<float>> doGPUEdgeSoftmax_4MultiCSCsKernel(
    std::vector<cusp::csr_matrix<int, int, cusp::device_memory>> csc_matrices,
    bool FlagInitWithRandomValue) {
  thrust::device_vector<int *> matCols_vect =
      thrust::device_vector<int *>(csc_matrices.size());
  ...
  std::vector<thrust::device_vector<float>> outNodes_per_relation_vect_vect(
      csc_matrices.size(),
      thrust::device_vector<float>(csc_matrices[0].num_rows, 0));
  thrust::device_vector<float *> outNodes_per_relation_vect;
  size_t total_nnzs = 0;
  for (int idx_relation = 0; idx_relation < csc_matrices.size();
       idx_relation++) {
    matCols_vect[idx_relation] = thrust::raw_pointer_cast(
        csc_matrices[idx_relation].column_indices.data());
    matRows_vect[idx_relation] =
        thrust::raw_pointer_cast(csc_matrices[idx_relation].row_offsets.data());
  ...
  for (int idx_relation = 0; idx_relation < csc_matrices.size();
    ...
    // outEdge_vect_for_curr_relation(csc_matrices[0].num_cols, 0);
  ...
  for (int idx_relationship = 0; idx_relationship < csc_matrices.size();
       idx_relationship++) {
    int num_blocks_for_this_and_prev_relation = (idx_relationship + 1 + 0.0) /
                                                (csc_matrices.size() + 0.0) *
                                                RTX_3090_GRIDSIZE;
  ...
  for (int idx_block = 0; idx_block < RTX_3090_GRIDSIZE; idx_block++) {
    if (idx_curr_relation < num_blocks_for_all_prev_relation_vect.size() - 1 &&
      ...
      assert(curr_beg_col_idx / RTX_3090_BLOCKSIZE ==
  ...
  std::chrono::high_resolution_clock::time_point t2 =
      std::chrono::high_resolution_clock::now();
  std::cout
      << "GPU HET_EdgeSoftmax_4MultiCSCsKernel time: "
